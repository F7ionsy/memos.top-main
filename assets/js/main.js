...
function updateHTMl(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        //提取位置信息
        const regex = /(?<=\*)[\u4e00-\u9fa5\w]+/; // 匹配*后面的汉字
        const matchResult = memoContREG.match(regex); // 尝试匹配文本中的内容
        // if (matchResult) {
        //     const locresult = matchResult[0]; // 如果匹配成功，提取匹配到的内容
        //     console.log(locresult); // 输出：海南大学海甸校区
        // }

        const index = memoContREG.indexOf("*");


        if (index === -1) {
            memoContREG = memoContREG;
        } else {
            memoContREG = memoContREG.substring(0, index);
        }
        ...
        if (data[i].resourceList && data[i].resourceList.length > 0) {
            ...
            for (var j = 0; j < resourceList.length; j++) {
                ...
                if (resType == 'image'&&resourceList[j].externalLink=="") {
                    imgUrl += '<img loading="lazy" src="' + memos + 'o/r/' + resourceList[j].id + '/' + resourceList[j].filename + '"/>'
                    resImgLength = resImgLength + 1
                }
				if (resType == 'image'&&resourceList[j].externalLink!==""&&resourceList.length!==1) {
                    imgUrl += '<img loading="lazy" class="flex9-child" src="' + resourceList[j].externalLink + '"/>'
                    resImgLength = resImgLength + 1
                }
                if (resType == 'image'&&resourceList[j].externalLink!==""&&resourceList.length==1) {
                    imgUrl += '<img loading="lazy" class="grid grid-" src="' + resourceList[j].externalLink + '"/>'
                    resImgLength = resImgLength + 1
                }
                if (resType !== 'image') {
            ...
            if (imgUrl) {
                ...
                if(resourceList.length!==1){memoContREG += '<div view-image class="flex9">' + imgUrl + '</div>'}
                if(resourceList.length==1){memoContREG += '<div view-image class="grid grid-">' + imgUrl + '</div>'}
            }
            if (resUrl) {
...
//获取 Memos 总条数
function getTotal() {
    var totalUrl = "https://memos.lihengdong.com/api/memo/stats?creatorId=1"
